...
class AmqpConsumerTest extends TestCase
{
    ...
    public function testConsumeKeepsTheBufferWhenAnotherTransportAlreadyDeliveredThisPass(): void
    {
        $channel = $this->createMock(AMQPChannel::class);
        ...
        $connection->method('isRegisteredWithWaitCoordinator')
            ->willReturn(true);
        $connection->method('hasDeliveredThisPass')
            ->willReturn(true);
        $connection->expects(self::never())
            ->method('waitForDeliveries');
        $connection->expects(self::never())
    ...
    public function testConsumeMarksDeliveredWhenYieldingABufferedEnvelope(): void
    {
        $channel = $this->createMock(AMQPChannel::class);
    ...
    public function testConsumeDoesNotMarkDeliveredWhenTheWaitYieldsNothing(): void
    {
        $channel = $this->createMock(AMQPChannel::class);
    ...
    public function testConsumeMarksDeliveredWhenTheWaitBuffersADelivery(): void
    {
        $channel = $this->createMock(AMQPChannel::class);
        ...
        $connection->expects(self::once())
            ->method('waitForDeliveries')
            ->with(1.5)
            ->willReturnCallback(static function () use ($consumer, $message): void {
                $consumer->callback($message);
            });
        $connection->expects(self::once())
            ->method('markDelivered');
